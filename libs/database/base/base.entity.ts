...
export abstract class BaseEntity<T> implements IBaseEntity<T> {
     ...
     */
    protected validateUuidFields(fields: { value: string; name: string }[]): void {
 ...
 */
export abstract class BaseEntityWithNumericId<T> implements IBaseEntityWithNumericId<T> {
    @PrimaryGeneratedColumn({
        comment: '기본키 (자동 증가)',
    })
    id: number;
