...
import { DataSource, EntityManager, EntityTarget, QueryFailedError, Repository } from 'typeorm';
import { IAggregateRoot, IDomainEvent } from './interfaces/repository.interface';
...
export class TransactionManagerService {
    ...
    private readonly errorHandlers = new Map<string, (error: DatabaseException) => Error | null>();
     ...
     * @returns 작업 결과
     */
    async executeTransaction<T>(operation: (manager: EntityManager) => Promise<T>, maxRetries: number = 3): Promise<T> {
     ...
     */
    async executeBatchTransaction<T>(operations: Array<(manager: EntityManager) => Promise<T>>): Promise<T[]> {
     ...
     * @returns 작업 결과
     */
    async executeDomainOperation<T>(operation: () => Promise<T>, context: string, domain: string): Promise<T> {
        const errorHandler = this.errorHandlers.get(domain);
        return this.executeSafeOperation(operation, context, errorHandler);
    }
     ...
     */
    private async processDomainEvents(events: IDomainEvent[], manager: EntityManager): Promise<void> {
